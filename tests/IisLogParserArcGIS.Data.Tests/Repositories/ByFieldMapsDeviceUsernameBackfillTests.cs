...
public class ByFieldMapsDeviceUsernameBackfillTests
{
    ...
    public void BackfillUsernames_FillsNullRowsOnEarlierAndLaterDatesFromTheDevicesLogin()
    {
        ...
        var repository = new ByFieldMapsDeviceRepository();
        repository.Insert(
            connection,
            [Row(_may1.AddDays(-10), DeviceA, null), Row(_may1, DeviceA, "user00364@somewhere"), Row(_may1.AddDays(10), DeviceA, null)]);

        var updated = ByFieldMapsDeviceRepository.BackfillUsernames(connection);
    ...
    public void BackfillUsernames_NeverOverwritesAnExistingUsername()
    {
        ...
        ByFieldMapsDeviceRepository.BackfillUsernames(connection);
    ...
    public void BackfillUsernames_CalledASecondTime_ChangesNothing()
    {
        ...
        var updatedOnSecondRun = ByFieldMapsDeviceRepository.BackfillUsernames(connection);
    ...
    private static List<string?> BackfilledUsernamesFor(ByFieldMapsDeviceAggregateRow[] rows)
    {
        using var database = new TempSqliteDatabase();
        using var connection = SqliteConnectionFactory.Open(database.Path);
        AggregateDatabaseSchema.EnsureCreated(connection);
    ...
    private static ByFieldMapsDeviceAggregateRow Row(DateOnly localDate, string deviceId, string? username) =>
        new()
        {
            LocalDate = localDate,
            DeviceId = deviceId,
